...
// MODIFIE
femProblem *femElasticityCreate(femGeo *theGeometry, double E1, double E2, double nu1, double nu2, double rho, double gx, double gy, femElasticCase iCase) {
  ...
  */
  ...
  if (iCase == PLANAR_STRESS) {
    // Matériau 1
    theProblem->A1 = E1 / (1 - nu1 * nu1);
    theProblem->B1 = E1 * nu1 / (1 - nu1 * nu1);
    theProblem->C1 = E1 / (2 * (1 + nu1));
    // Matériau 2
    theProblem->A2 = E2 / (1 - nu2 * nu2);
    theProblem->B2 = E2 * nu2 / (1 - nu2 * nu2);
    theProblem->C2 = E2 / (2 * (1 + nu2));
  } else if (iCase == PLANAR_STRAIN || iCase == AXISYM) {
    // Matériau 1
    theProblem->A1 = E1 * (1 - nu1) / ((1 + nu1) * (1 - 2 * nu1));
    theProblem->B1 = E1 * nu1 / ((1 + nu1) * (1 - 2 * nu1));
    theProblem->C1 = E1 / (2 * (1 + nu1));
    // Matériau 2
    theProblem->A2 = E2 * (1 - nu2) / ((1 + nu2) * (1 - 2 * nu2));
    theProblem->B2 = E2 * nu2 / ((1 + nu2) * (1 - 2 * nu2));
    theProblem->C2 = E2 / (2 * (1 + nu2));
  }
...
void femElasticityPrint(femProblem *theProblem) {
  ...
  printf("Material 1 : \n");
  printf("   Young modulus E1   = %14.7e [N/m2]\n", theProblem->E1);
  printf("   Poisson's ratio nu1  = %14.7e [-]\n", theProblem->nu1);
  printf("Material 2 : \n");
  printf("   Young modulus E2   = %14.7e [N/m2]\n", theProblem->E2);
  printf("   Poisson's ratio nu2  = %14.7e [-]\n", theProblem->nu2);
  printf("For both materials :\n");
...
void femElasticityWrite(femProblem *theProblem, const char *filename) {
  ...
  fprintf(file, "Young modulus 1    : %14.7e  \n", theProblem->E1);
  fprintf(file, "Poisson ratio 1    : %14.7e  \n", theProblem->nu1);
  fprintf(file, "Young modulus 2    : %14.7e  \n", theProblem->E2);
  fprintf(file, "Poisson ratio 2    : %14.7e  \n", theProblem->nu2);
...
femProblem *femElasticityRead(femGeo *theGeometry, const char *filename) {
  ...
  while (!feof(file)) {
    ...
    if (strncasecmp(theLine, "Young modulus 1     ", 19) == 0) {
      ErrorScan(fscanf(file, ":  %le\n", &theProblem->E1));
    }
    if (strncasecmp(theLine, "Young modulus 2     ", 19) == 0) {
      ErrorScan(fscanf(file, ":  %le\n", &theProblem->E2));
    }
    if (strncasecmp(theLine, "Poisson ratio 1     ", 19) == 0) {
      ErrorScan(fscanf(file, ":  %le\n", &theProblem->nu1));
    }
    if (strncasecmp(theLine, "Poisson ratio 2     ", 19) == 0) {
      ErrorScan(fscanf(file, ":  %le\n", &theProblem->nu2));
    }
  ...
  theProblem->nu = nu;
  */
  double E1 = theProblem->E1;
  double E2 = theProblem->E2;
  double nu1 = theProblem->nu1;
  double nu2 = theProblem->nu2;
